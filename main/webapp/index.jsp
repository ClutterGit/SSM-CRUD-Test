...
<head>
    ...
    以/开始的相对路径，找资源，以服务器的路径为标准 ====推荐使用====
    --%>
    <script src="${APP_PATH}/static/bootstrap/js/jquery.min.js"></script>
    <!--引入bootstrap前端框架的css、js-->
...
<body >
<%--员工修改模态框--%>
<div class="modal fade" id="empUpdateModal" tabindex="-1" role="dialog" aria-labelledby=" ">
...
<!-- 员工添加模态框 -->
<div class="modal fade" id="empAddModal" tabindex="-1" role="dialog" aria-labelledby=" ">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
<script type="text/javascript">
    ...
    function getDepts(ele){
        ...
        $.ajax({
            ...
            success:function (result){
                //{"code":100,"msg":"处理成功！",
                // "extend":{"depts":[{"deptId":1,
                // "deptName":"开发部"},{"deptId":2,"deptName":"测试部"}]}}
                //显示部门信息在下拉列表中
                //$("#dept_add_select") 等于
            ...
            success:function (result){
               //alert(result.msg);
                if(result.code == 100){
                    ...
                    //显示失败信息
                    //有哪个字段的错误信息就显示哪个字段的
                    if(undefined != result.extend.errorFields.email){ //如果email的错误信息不是未定义
                        //显示邮箱的错误信息
                        show_validate_msg("#email_add_input","error",result.extend.errorFields.email)
                    }
                    if(undefined != result.extend.errorFields.empName){ //如果empName的错误信息不是未定义
                        //显示员工名字的错误信息
    ...
    $(document).on("click",".delete_btn",function (){
        ...
        var empId = $(this).attr("del-id"); //给删除按钮这条信息的id值
